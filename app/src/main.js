...
import { catalogForTopic, displayRanking, filterRanking, initials, nextBalancedPair, rankingForCatalog, shortName, voteFeedback } from "./domain.js";
...
function rankingScreen() {
  ...
  const rows = visible.map((person) => `<button class="ranking-row" type="button" data-profile="${escapeHtml(person.id)}"><strong>${person.displayRank ?? "—"}</strong><span>${escapeHtml(person.displayName || shortName(person.name))}<br><small>${person.decisions ? `${person.wins} escolhas · ${person.losses} não escolhas` : "Ainda sem duelos"}</small></span><strong>${person.decisions ? `${person.winRate}%` : "—"}</strong></button>`).join("");
  ...
  return `<main class="screen"><div><p class="eyebrow">Eleições 2026</p><h1>Ranking</h1><p class="lead">${personal ? "Sua ordem é calculada apenas pelas comparações que você fez." : "Preferência coletiva calculada com votos confirmados pelo servidor."}</p><p class="ranking-total">${totalDuels} ${totalDuels === 1 ? "duelo confirmado" : "duelos confirmados"}</p></div>${state.result ? `<div class="result-banner" role="status">${escapeHtml(state.result)}</div>` : ""}<div class="segmented" aria-label="Tipo de ranking"><button class="${personal ? "" : "active"}" data-ranking-view="general">Geral</button><button class="${personal ? "active" : ""}" data-ranking-view="personal">Seus escolhidos</button></div>${podiumCards ? `<section class="podium" aria-label="Pódio">${podiumCards}</section>` : ""}<label class="ranking-search"><span>Buscar no ranking</span><input id="ranking-search" type="search" value="${escapeHtml(state.rankingQuery)}" placeholder="Nome ou partido" autocomplete="off"></label><section class="panel ranking-list">${rows || `<p class="empty">${empty}</p>`}</section>${reveal}<button class="primary continue-duels" id="continue-duels" type="button">Voltar aos duelos</button></main>`;
}
