import { PARTY_CODES, PRIMARY_AREAS } from "./catalog-taxonomy.js";
...
export function sessionMirror(session) {
  ...
  const axis = (id, label, field, vocabulary) => {
    const known = chosen.filter(person => person.publication?.content?.status === "approved" && person.taxonomyProvenance?.[field]?.status === "extracted" && vocabulary.includes(person[field]));
  ...
  return { version: 1, editionId: session.edition.id, total,
    axes: [axis("area", "Áreas de atuação", "primaryArea", PRIMARY_AREAS), axis("party", "Partidos", "party", PARTY_CODES),
