...
## Ajuste de tipografia e densidade — 22/09/2026

Stefan revisou as capturas do protótipo mesclado e pediu para reduzir o tamanho das letras e a densidade visual ("nada de minimalista"). Ajuste em `app/src/civic/ui/civic.css`: fontes menores em todos os níveis (título da ficha 1.5rem→1.2rem; cards/seções/notícias ~1–1.1rem→.88–.96rem; corpo/metadados ~.74–.9rem; labels/chips ~.66–.72rem), padding/gaps reduzidos ~15–20%, raio de borda mais discreto (18–22px→10–14px) e sombra do cartão bem mais sutil. Altura mínima de toque de 44px preservada em botões e campos. 199/199 testes unitários e 13/13 cenários de `civic-navigation.mjs` seguiram passando; o teste de larguras iguais dos cartões não depende de valor fixo. PR [#240](https://github.com/stefancabral-oss/presidencia-duelo/pull/240), mesclada por Stefan (squash) em `main` no commit `55b21c2`, confirmando o resultado visual.
