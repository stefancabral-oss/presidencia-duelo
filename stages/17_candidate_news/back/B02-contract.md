...
Módulos novos em back/src/civic-import.js, civic-acquisition.js, civic-import-store.js e civic-divulga.js. São executados explicitamente por CLI; não são importados pelo servidor, não montam rotas nem escrevem no PostgreSQL. Staging usa arquivos locais e exporta o contrato civic v1 para a futura ingestão autorizada por B06. Migração B01 permanece independente.
...
32 cenários de regressão exercitam módulos reais, CLI real, ZIP/CRC real em Python, cortes por eleição, homônimos, suplentes, vínculos conflitantes, ausência de vigência, idempotência, locks, arquivos truncados e preservação da última carga completa. Suíte local completa: 431 testes aprovados em execução serial e build aprovado. Resultados e comandos em [B02-verification.json](../output/B02-verification.json). CI remoto e revisão humana são conferidos na PR.
