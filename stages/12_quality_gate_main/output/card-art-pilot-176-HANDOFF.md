...
- Schema consolidado sem respostas individuais nem narrativa pública livre, com supressão de células pequenas, revisão humana de identidade/dignidade, responsável/data da decisão e custódia obrigatória do conjunto de respostas. `notes` e `rationale` não existem; outcomes e motivos são códigos fechados.
...
- Relógio injetável com tolerância de cinco minutos exige `manifest.generatedAt` < coleta ≤ atestação ≤ revisões ≤ decisão ≤ consolidado e bloqueia futuro. `attestedBy`, `reviewedBy` e `decidedBy` aceitam apenas IDs opacos `gov_` + 32 hexadecimais, separados do cadastro privado dos responsáveis e de qualquer PII de participante. Outcomes e `reasonCodes` são listas fechadas e compatíveis com status/decisão. Eventual narrativa humana fica em artefato privado/redigido fora do bundle público; o consolidado guarda somente `artifactId`, SHA-256 e marcador fixo de tratamento. A varredura NFKC/casefold PT-BR permanece como defesa em profundidade, não como autorização de texto livre.
...
9. Gerar todas as métricas pelo derivador e validar com `card-art-pilot:validate-results -- <resultado.json> <response-bundle.json>`; registrar revisões/atestação/decisão somente com IDs opacos e códigos fechados. Se houver narrativa, mantê-la em artefato privado/redigido e publicar apenas sua referência + hash; só depois apagar os dados individuais transitórios.
