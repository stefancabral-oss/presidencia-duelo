...
test("the daily edition is identical for every player and reproducible from date plus sorted catalog", () => {
  ...
  assert.equal(firstPlayer.catalogSchema, "candidate-public-v4");
...
test("ruleset fixes ten daily choices plus twenty free choices in the same editorial day", () => {
  ...
  assert.equal(DAILY_SESSION_RULESET, DAILY_SESSION_RULESET_V4);
  assert.equal(DAILY_SESSION_RULESET.catalogSchema, "candidate-public-v4");
  ...
  assert.equal(dailyRulesetByIdentity("daily-four-card-v4", 4), DAILY_SESSION_RULESET_V4);
  assert.throws(() => dailyRulesetByIdentity("daily-four-card-v5", 5), /histórico não suportado/);
});
