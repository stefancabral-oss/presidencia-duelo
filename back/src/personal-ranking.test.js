...
test("pending comparisons build the transactional after-snapshot before persistence", () => {
  ...
  const serializedScores = JSON.parse(JSON.stringify(result)).ranking.map(({ preferenceScore }) => preferenceScore);
  assert.deepEqual(serializedScores, result.ranking.map(({ preferenceScore }) => preferenceScore));
});
