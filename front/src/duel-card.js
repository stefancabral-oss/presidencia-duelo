...
/**
 * Card visual compartilhado pelo duelo. A estrutura preserva seletores legados
 * usados pela lógica atual, mas passa a expor a anatomia do novo design system.
 */
export const DUEL_CARD_SKELETON = `
      ...
      <div class="card-bottom card-legacy-metrics" hidden>
...
function descriptionText(candidate) {
...
export function fillDuelCard(el, candidate, { elo, wr, barWidth }) {
  ...
  if (role) role.textContent = roleText(candidate);
  const description = el.querySelector(".pm-card__description");
  if (description) description.textContent = descriptionText(candidate);
